...
legacy `state: yellow`, `expires_at` has passed, or its `expires_every` duration after `checked_at` has elapsed. A legacy `verified_at` is accepted as a fallback. Agents
...
clears sticky `falsified_at` and writes
